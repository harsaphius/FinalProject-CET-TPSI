...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script>
        function handleLinkButtonClick(action) {
            ...
            setPageTitleAndSiteMapNode(action, title);

            // Return false to prevent default postback behavior
            return false;
        }

        function setPageTitleAndSiteMapNode(action, title) {
            // Call a server-side method or handler using AJAX
            $.ajax({
                type: "POST",
                url: "ManageClassrooms.aspx/SetPageTitleAndSiteMapNode", // Update the URL with your actual page and method name
                data: JSON.stringify({ action: action, title: title }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    // Handle the response if needed
                    console.log("Page title and SiteMapNode updated successfully.");
                },
                error: function (xhr, status, error) {
                    // Handle any errors
                    console.error("Error updating page title and SiteMapNode:", error);
                }
            });
        }
    </script>
